...
export default function Chart({ data }: any) {
  const { selectedStockIndex, turn, startDate, setStartDate, endDate, setEndDate, isBuySellModalOpen, assetListData, tradeListData, todayStockInfoListData } = FundGameStore();
  ...
  useEffect(() => {
    ...
    // 차트를 담을 컨테이너 생성
    const container = chart.container("fund-chart-container")
    ...
    <div className="row-span-12 grid grid-rows-12">
        <div className="row-span-1 grid grid-cols-8 items-center">
            ...
            <button 
                ...
                className={`border ${selectedSecondaryIndicator == 1 ? 'bg-slate-400 text-white border-slate-400' : 'border-black'} m-1 px-1 rounded-md`}
            >
                Volume
            </button>
            <button 
                ...
                className={`border ${selectedSecondaryIndicator == 2 ? 'bg-slate-400 text-white border-slate-400' : 'border-black'} m-1 px-1 rounded-md`}
            >
                RSI
            </button>
        <button 
            ...
            className={`border ${selectedSecondaryIndicator == 3 ? 'bg-slate-400 text-white border-slate-400' : 'border-black'} m-1 px-1 rounded-md`}
        >
            MACD
        </button>
        <button 
            ...
            className="border border-black m-1 px-1 rounded-md"
        >
            전체보기(`)
        </button>
    </div>    
    <div id="fund-chart-container" className="row-span-12 flex items-center justify-center"></div>
</div>
